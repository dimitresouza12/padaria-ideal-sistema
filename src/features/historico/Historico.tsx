...
/**
 * Histórico de vendas. Quando `restritoVendedorId` é passado (perfil vendedor),
 * lista só as vendas daquele vendedor e esconde o filtro de vendedor.
 */
export function Historico({ restritoVendedorId }: { restritoVendedorId?: string } = {}) {
  const { vendas, usuarios, comercios, produtos } = useDataStore();

  const [vendedorId, setVendedorId] = useState(restritoVendedorId ?? TODOS);
  ...
  const filtradas = useMemo(() => {
    return vendas
      .filter((v) => !restritoVendedorId || v.vendedor_id === restritoVendedorId)
  ...
  }, [vendas, restritoVendedorId, vendedorId, comercioId, produtoId, status]);
  ...
  return (
    <div className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 gap-3 sm:flex sm:flex-wrap sm:items-end">
          {!restritoVendedorId && (
            <div className="w-full sm:w-auto">
              ...
              <select className="field w-full sm:!w-44" value={vendedorId} onChange={(e) => setVendedorId(e.target.value)}>
          ...
          <div className="w-full sm:w-auto">
            ...
            <select className="field w-full sm:!w-44" value={comercioId} onChange={(e) => setComercioId(e.target.value)}>
              <option value={TODOS}>Todos</option>
          ...
          <div className="w-full sm:w-auto">
            ...
            <select className="field w-full sm:!w-44" value={produtoId} onChange={(e) => setProdutoId(e.target.value)}>
              <option value={TODOS}>Todos</option>
          ...
          <div className="w-full sm:w-auto">
            ...
            <select className="field w-full sm:!w-36" value={status} onChange={(e) => setStatus(e.target.value as typeof status)}>
          ...
          <div className="col-span-2 text-[12.5px] text-ink-muted sm:ml-auto sm:text-right">
            <div>{filtradas.length} venda(s) · <span className="font-bold tabular-nums text-ink">{fmtBRLCompact(totalFiltrado)}</span></div>
            <div className="text-[11px]">Toque numa venda para ver os detalhes.</div>
          </div>
        </div>
      </Card>
      ...
      <div className="flex flex-col gap-3 sm:hidden">
        {filtradas.length === 0 ? (
          <Card><EmptyState>Nenhuma venda encontrada com esses filtros.</EmptyState></Card>
        ) : (
          filtradas.map((v) => (
            <Card key={v.id} className="cursor-pointer p-4 transition active:bg-plane" onClick={() => setSelecionada(v)}>
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="text-[13.5px] font-semibold">{nomeProduto(v.produto_id)}</div>
                  <div className="text-xs text-ink-muted">{nomeComercio(v.comercio_id)} · {nomeVendedor(v.vendedor_id)}</div>
                </div>
                {statusTag(v.status)}
              </div>
              <div className="mt-2.5 flex items-center justify-between border-t border-line pt-2.5 text-[12.5px]">
                <span className="tabular-nums text-ink-muted">{fmtData(v.data_venda)} · {v.quantidade} cx</span>
