...
/**
 * Substituto de `<select>` para listas longas (ex.: clientes, produtos) — filtra
 * as opções conforme o usuário digita, em vez de rolar um dropdown nativo enorme.
 * Navegação por teclado (setas + Enter + Escape) para não regredir a UX do select.
 */
export function ComboBox({
  ...
  const escolher = (o: ComboBoxOpcao) => {
    onChange(o.value);
    setAberto(false);
    setBusca('');
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      />
      {aberto && (
        <div className="absolute z-20 mt-1 max-h-56 w-full overflow-y-auto rounded-lg border border-line-strong bg-white py-1 shadow-cardlg">
          {filtradas.length === 0 ? (
            <div className="px-3 py-2 text-[12.5px] text-ink-muted">Nenhum resultado</div>
          ) : (
            filtradas.map((o, i) => (
              <button
                key={o.value}
                ...
                className={`block w-full truncate px-3 py-2 text-left text-[13px] transition ${
                  i === destaque ? 'bg-accent-wash text-accent-dark' : 'hover:bg-plane'
                }`}
              >
                {o.label}
              </button>
